...
##############################################################
#
# Ask questions to prepare cloud-init file for Raspberry Pi 5
#
# if ~/.mcu_experiments_sd.txt exists no questions will be asked
#
##############################################################
...
def ask_questions():
    ...
    if 'ubuntu_password' not in target_environment.keys():
        ask_user("Ubuntu user password", 'ubuntu_password')
...
parser = argparse.ArgumentParser(description='Prepare SD')
parser.add_argument('-c', '--cache',
                    action='store_true',
                    help='Cache my responses')
args = parser.parse_args()

conf_file = os.path.join(os.path.expanduser("~"), '.mcu_experiments_sd.txt')
