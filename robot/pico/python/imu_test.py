...
params.add_param('vz', 'float', 1)
params.add_param('orientation', 'float', 1)
...
while True:
    ...
    if (now - last_called) > 100:
        last_called = now
        params.tele_plot.send_value("vz", params.robot_params['vz']['value'][0], unit='rad/sec')
        params.tele_plot.send_value("orientation", params.robot_params['orientation']['value'][0] / pi * 180, unit='deg')
